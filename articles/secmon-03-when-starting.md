...
- **既製プロダクト**: 専用のハードウェアで構成されたセキュリティ監視プロダクトや、ソフトウェア製品をベンダーから購入し、汎用サーバーやクラウド上のインスタンスなどに導入するものを指します。
...
既製プロダクトは導入に多くの購入費用がかかります。ほとんどの場合、ハードウェアやソフトウェア利用のライセンスを最初に購入する必要があります。規模にもよりますが、数百万円から数千万円単位の購入費用がかかることが一般的です。また、導入工数も多くかかることが多いです。これは接続するためのネットワークの調整や設定の変更、プロダクトの検証、そしてその上での実際の導入作業が必要になるためです。さらに、既製プロダクトの場合、カスタマイズや拡張に工数がかかることが多く、開発工数もある程度見込んでおく必要があります。
