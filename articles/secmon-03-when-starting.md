...
セキュリティ監視は情報セキュリティ対策の一環として重要な要素ですが、始めるべきタイミングは組織や事業規模によって異なります。今回は、セキュリティ監視基盤をいつから始めるべきかについて考察します。

# 組織の脅威・リスク分析をする

セキュリティ監視とは数ある情報セキュリティ対策の手段の一つにすぎません。セキュリティ監視に限らず、どのような情報セキュリティ対策に取り組むかは、組織の情報資産・守るべきデータにどのようなものがあるかを把握し、さらに脅威やリスクを理解し、そこからどのように対処するか（あるいは対処しないか）を考えることから始まります。

本文書では脅威・リスク分析については詳しく触れませんが、セキュリティ監視を導入するかどうかを判断するためには、脅威・リスク分析を行うことが重要です。以下に脅威・リスク分析として著名なフレームワークやガイドラインをいくつか紹介します。
...
- [MITRE ATT&CK](https://attack.mitre.org/): MITREが開発したフレームワークで、サイバー攻撃の戦術や技術を体系的に整理したフレームワークです。攻撃者の行動を理解し、防御策を強化するために利用されます。
- [NIST SP 800-30](https://csrc.nist.gov/pubs/sp/800/30/r1/final): NISTが提供するリスク評価のためのガイドラインです。リスクの特定、評価、対処を行うための手法が記載されています。

このようなフレームワークやガイドラインを参考に、組織の情報資産やリスクを把握し、それの対処方法としてセキュリティ監視が適しているかどうかを判断することが重要です。

# セキュリティ監視に関するコスト

脅威・リスクの分析まで行った上で、対策としてセキュリティ監視を導入するかどうかを判断するための重要な要素として、セキュリティ監視にかかるコストの問題があります。セキュリティにおける防御的対策は、一度設定をしてしまえばそのあとは稀に調整をするだけですむことが多いです。一方で、セキュリティ監視は導入のためのコストだけでなく、継続的な運用コストがかかります。このコストに向き合えるかどうかが、セキュリティ監視を導入するかどうかの判断材料として重要になります。

セキュリティ監視にかかるコストは、大きく分けて導入と運用の2つに分かれます。さらに既製プロダクト、既製サービスの利用と内製でもコストが異なります。

- **既製プロダクト**: 専用のハードウェアで構成されたセキュリティ監視プロダクトや、あるいはソフトウェア製品をベンダから購入し、汎用サーバーやクラウド上のインスタンスなどに導入するものを指します。
- **既製サービス**: クラウドサービスとして提供される、セキュリティ監視サービスを利用するものを指します。
...
:::message
あくまで大まかな目安としての比較です。実際のコストは、導入時の条件や状況、運用時の状況、契約によって異なります。
:::

| 項目 | 既製プロダクト | 既製サービス | 内製 |
| --- |:---:|:---:|:---:|
| 購入費用 | 数百〜数千万 | ほぼ無し | ほぼ無し |
| 導入工数 | 2〜3人月 | 1〜10人日 | 1〜10人日 |
| 開発工数 | 2〜3人月 | 1〜10人日 | 5〜20人月 |

まず、既製プロダクトは導入に多くの購入費用がかかります。ほとんどの場合はハードウェアあるいはソフトウェア利用のライセンスを最初に購入する必要があります。規模にもよりますが、数百万円から数千万円単位の購入費用がかかることが一般的です。また、導入工数も多くかかるような状況になりがちです。これは接続するためのネットワークの調整や設定の変更、プロダクトの検証、そしてその上での実際の導入作業が必要になるためです。さらに（一概には言えませんが）既製プロダクトの場合、カスタマイズや拡張に工数がかかることが多く、開発工数もある程度見込んでおく必要があります。

一方、既製サービスは従量課金型が多く、購入費用がほぼかかりにくいです。さらにクラウドサービスの場合、アカウントを作成し、設定を行うだけで利用できることが多く導入工数も比較的安価ですみます。拡張性も一定考慮されており、独自サービス・アプリからログを取り込む場合の開発工数は、既製プロダクトに比べて少ないことが多いです。ただし逆に言うと、対応できない機能についてはいくら工数をかけても対応できないことが多く、この点については注意が必要です。
