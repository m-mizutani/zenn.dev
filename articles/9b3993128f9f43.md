...
- `ShouldMask(fieldName string, value interface{}, tag string) bool`: 検査する値のフィールド名、値そのもの、そして構造体に `zlog` のタグがついていればその情報が引数として渡されます。返り値が `false` の場合は何もせず、 `true` だった場合はそのフィールドが全て隠蔽されます。隠蔽された値は元の型によらず、全て `string` 型の `[filtered]` という値に置き換えられます。
