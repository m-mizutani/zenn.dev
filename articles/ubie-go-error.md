...
Go言語で開発したことがある方はご存知かと思いますが、Goは標準パッケージで提供されているエラーハンドリングは最低限の機能しか提供されていません。これは、CLIツールなどではエラーの内容が簡潔に表せてよいのですが、サーバサイドアプリケーションのようにエラーにまつわる情報を詳細に残してあとから調査に利用する、という場面では不向きです。特に本番環境でしか再現しないようなエラーの場合は、いかに関連情報を残せているかが、問題の解決に大きく影響します。
...
しかしこの方法だとエラーの種類を示す情報とエラーに関連する情報が一つの文字列に混ざり合ってしまいます。これによって、例えば、エラーの種類ごとに集計や集約をしたい場合、エラーメッセージをパースしてエラーの種類を取り出す必要があります。値の埋め込み方はエラーによって様々なので、パースの方法もエラーごとに異なり、これが集約や集計をさらに難しくしています。

このことからエラーに関連する情報とエラーの種類を示す情報は分離して扱うのが望ましいと考えられます。
...
`goerr` は `With(key, value)` というメソッドを使ってエラーに関連情報を付与することができます。これによって、エラーの種類と関連情報の見分けやすくなったり、集計・集約が容易になります。値は `Values()` というメソッドで取り出すことができます。
...
先述した通り、`Stacks()` と `Values()` でエラーのスタックトレースと関連情報を構造化した状態で取り出すことができます。これを利用して構造化ログを出力することはできますが、プロジェクトごとにエラーを構造化するのは面倒です。そこで、`goerr` は [slog.LogValuer](https://pkg.go.dev/golang.org/x/exp/slog#LogValuer) インターフェースを標準で実装しています。 これによって `goerr.Error` 型をそのままログの引数として指定するだけで、エラーを構造化ログとして出力することができます。

この出力ではスタックトレースや関連情報も含まれます。また、Wrapされたエラーについても再帰的に内容を出力するようにしています。
