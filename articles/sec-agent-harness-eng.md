...
また、エージェントが外部にデータを持ち出す経路についても考慮しています。Warrenのツール群の中で外部への通信が可能なのは実質的にWebFetchのみであり、前述のとおりHITLによる人間の承認を必須としています。Slack検索やBigQueryなど他のツールはデータの読み取り専用であり、外部への送信経路を持ちません。つまりWebFetchという単一のチョークポイントを押さえることで、データ漏洩のリスクを構造的に抑えています。
