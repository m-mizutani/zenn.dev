...
_今回対象となるWarrenは各サービス・監視装置から受け取ったアラートを分析するための生成AIエージェントになります_
...
- **計測観点**：エージェントの挙動を把握・評価できるようにすること。何をしているか・どこで詰まっているかの追跡と、出力品質の評価が含まれる
- **実行設計観点**：1回の実行を「ちゃんとした形で動かす」ための設計。コンテキストの効率化、実行フローやツール選択による制御、権限管理や実行安全性の確保、セッションの永続化と耐障害性などが含まれる
- **実行最適化観点**：1回の実行の品質・速さ・安さを改善すること。エージェント性能の向上、レイテンシ・スループットの改善、コスト効率化が含まれる
...
このセクションのプロンプトは最初から完成していたわけではなく、運用中に観察された問題への対処として段階的に追加されてきたものです。特に大きかったのは確証バイアスへの対策で、エージェントがアラートの深刻度を過大評価して恐怖を煽るか、逆に過小評価して安心させすぎるという両極端な傾向が観察されました。これに対して「恐怖の増幅でも偽りの安心でもなく、最も蓋然性の高いシナリオに基づいて評価せよ」という基本原則をプロンプトに追加し、約210行の大幅な改訂を行っています。

そこから「事実と仮説を明確に区別せよ」「複数の解釈を考慮せよ」「反証を積極的に探せ」といった分析姿勢の強制を行っています。Anti-Patternsセクションでは「やってはいけないこと」を具体例付きで列挙しており、確証バイアスの抑止や調査報告書スタイルの禁止などが含まれます。

例えば分析に関する戦略として、以下のようなプロンプトを与えています（[全文](https://github.com/secmon-lab/warren/blob/main/pkg/usecase/chat/bluebell/prompt/system.md)）
...
すべての安全装置の前提として、ツール実行のアーキテクチャ自体がクレデンシャルをLLMから隔離する設計になっています。Warrenでは各外部サービス（BigQuery、Slack、CrowdStrike Falconなど）へのアクセスをFactory パターンで構成しており、Factoryがクレデンシャルを受け取ってクライアントを生成し、LLMに公開されるToolSetインターフェースにはパラメータの仕様と実行エントリポイントしか含まれません。LLMが指定できるのはSQLクエリや検索キーワードなどのパラメータだけであり、APIキーやサービスアカウント、OAuthトークンといったクレデンシャルはGoの構造体フィールドに閉じ込められてLLMのコンテキストには一切現れません。つまりプロンプトインジェクションなどでLLMの挙動が操作されたとしても、クレデンシャルそのものを窃取・漏洩させることはアーキテクチャ上不可能となっています。

#### 自動的なガードレール

この土台の上に、ツール実行に対する自動的な安全装置を設けています。代表例はBigQueryクエリに対する多層的なガードレールです。全クエリを実行前にドライランし、設定されたスキャンサイズ上限を超える場合は実行をブロックします。これはBigQueryスキャン破産防止用です。結果行数も最大1000行で切り捨てており、超過時は「OFFSETでページネーションするな、GROUP BYで集約せよ」とエージェントに指示します。こちらはどちらかというとコンテキスト消費を効率化するための工夫です。クエリタイムアウトはデフォルト5分でジョブをキャンセルします。さらにプロンプトでも `SELECT *` 禁止、`OFFSET` 禁止などのアンチパターンを明示し、構造的制約とプロンプト制約の両面で防御しています。加えて、プロンプトインジェクション経由で悪意あるクエリ（`DELETE`や`DROP`など）を実行させるシナリオに対しては、BigQueryに接続するサービスアカウントの権限を`dataViewer`に絞ることで対処しています。仮にLLMが破壊的なクエリを生成したとしても、権限不足で実行が拒否されます。

#### 攻撃耐性：データ漏洩とプロンプトインジェクション

エージェントが外部にデータを持ち出す経路についても考慮しています。Warrenのツール群の中で外部への通信が可能なのは実質的にWebFetchのみであり、HITLによる人間の承認を必須としています。Slack検索やBigQueryなど他のツールはデータの読み取り専用であり、外部への送信経路を持ちません。つまりWebFetchという単一のチョークポイントを押さえることで、データ漏洩のリスクを構造的に抑えています。

入力側のプロンプトインジェクションについても触れておきます。Warrenは分析のためにWeb上のデータや外部アラートなど、必ずしも信頼できないデータも取り込みます。そのため悪意あるアラートデータにプロンプトインジェクションが仕込まれる可能性は排除できません。しかし前述のとおり、エージェントから外部への干渉手段が原則として存在しないため、仮にプロンプトが汚染されても「分析結果が歪められる」ことが主なリスクとなります。この点については、エージェントの分析経過がすべてトレースとして記録・表示されるため、人間が結果を検証し、不自然な判断を追跡できるようになっています。

#### 人間による監視と運用制御

HITL（Human-in-the-Loop）の仕組みとして、特定のツール呼び出しに対して人間の承認を必須にでき、Slackの対話的ボタン（Approve / Deny）で承認フローを実現しています。Presenterが未設定の場合はツール実行自体をブロックし、承認ポリシーのバイパスを防止します。今のところおもにWebFetchツールで利用しています。さらにリプランフェーズでは、ツールだけでは情報が不足する場合にオペレーターに質問を投げかけることもできます。質問には選択肢と理由を必須で付与してオペレーターが判断しやすい形式にしており、回答は次のリプランに引き継がれて調査方針の修正に使われます。

運用面では、Slackコマンドでエージェントの実行を中断でき、中断要求はセッションのステータス変更（`running` → `aborted`）としてFirestoreに記録されます。エージェントはツール呼び出しの境界でステータスをチェックし、中断を検知すると実行を停止します。対象チケットに紐づくすべての実行中セッションが一括で中断されますが、即座に停止するのではなく「次のチェックポイントで停止」する設計のため、実行中のツール呼び出しは安全に完了します。アクセス制御についてはRegoポリシーによるSlackユーザーベースの認可を導入しており、社内サービスのアクセスログやEDRの監視ログなど機微な情報が含まれる可能性があるデータを利用するため、実行権限を持つユーザを制限しています。いざとなったらSlackメッセージ削除コマンドで一通り関連メッセージを消去できる機能も搭載しています。
...
上述した現在のナレッジシステムに至るまでに、2つの前身となる記憶システムを構築し、いずれも全廃止しています。最初は「実行後にとりあえず覚えておくべきことを漠然と覚えさせるだけの」記憶機能、もう一つは専用エージェント（例えばBigQuery専用に動作するエージェント）毎に記憶をもたせるという方式です。それぞれ記憶を新しく生成するだけで整理がなかった点が特徴です。それを踏まえて、現状では1 shotで記憶を生成するのではなく、記憶の検索・編集ツールを渡したエージェントに「記憶の追記・編纂」作業をやらせるようにしました。こうすることで単に記憶を追加・削除するだけでなく、既存の知識と統合したり、間違った記憶を削除したりなど、常に整理と編纂をして記憶を膨張させないようにしたのが大きな特徴と考えています。
...
またアラートの受信時には、Ingest（アラートの整形や受入判定） → Enrich（分析に必要な情報の事前取得） → Triage（影響度判定） の3段階パイプラインでアラートを自動処理しています。各段階でイベントを発行して外部からの監視を可能にしつつ、Regoポリシーで処理の要否を制御することで、不要なアラートはエージェント呼び出し前にフィルタリングされます。

### コスト効率化

前述の思想で述べたとおり、セキュリティ調査はいくらでも深掘りできてしまうため、明示的にコストを制御する仕組みが必要です。もともとWarrenでは子エージェントでループの上限だけ決めて行動させていました。しかしそれだと深堀りしようとした結果途中で止まり、なんの結果も得られないという処理が度々見られました。また時間のかかる処理を連発し、なかなか結果が返ってこないということもあり、時間の概念も反映する必要があるという考えに至りました。

現在のWarrenではバジェット戦略によってタスクの打ち切りを実現しています。各タスクにバジェット（デフォルト100ポイント）を設定し、ツール種別ごとのコストと時間コスト（経過秒数ベース）で消費していきます。ソフトリミット（バジェット枯渇）で警告、ハードリミット（ソフトリミット後3回超過）で強制終了という二段構えになっており、強制終了時はハンドオーバー情報（ツール履歴、経過時間、消費割合）を生成してリプランフェーズでより小さなタスクに分割されます。BigQuery・Falcon・Slackの子エージェント呼び出しは親タスクのバジェットから除外し、内部で個別に追跡することでコストを分離しています。
