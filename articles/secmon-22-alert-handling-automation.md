...
条件分岐ではなく全てのパターンに応じたワークフローを実装することも考えられますが、その場合はワークフローの数が多くなることで管理が複雑になり、メンテナンスなどの負担が大きくなるという懸念があります。
...
今回構築するセキュリティ監視基盤の要件などを踏まえて、アラート対応の自動対応の仕組みを実装するにあたって考慮するべきポイントをまとめてみました。アラート対応は全て自動化することを目的にするのではなく、自組織やチームなどの状況を踏まえつつ、対応にコストがかかっている部分を「エンジニアリング」して効率化していく、というアプローチが望ましいのではないかと考えます。
