...
今回は、これまで解説したログ収集フェーズの議論を元に、Pull型によるログ収集・保全の実装の例を紹介します。

# ログ収集・保存のフレームワーク化

ログ収集の記事でも紹介した通り、Pull型のログ収集はAPIにアクセスして収集したデータを保存する、というとてもシンプルな処理になります。そのため簡単なスクリプトをいくつか書くだけで良いと考えられがちなのですが、実際にはログ収集の解説で述べたように、細かに気にしなければならないポイントがいくつかあります。また、ログ収集の処理は外部サービスごとに違うため、それぞれのサービスに対して個別のスクリプトを書くとサービスの数だけ実行スクリプトが増えてしまいます。これらにすべて異なる実行環境を構築していると、管理が煩雑になります。

そこで今回の例では、ログ収集・保存の処理をフレームワーク化することで、管理を統合するというアプローチをとりました。ログを収集する部分は外部サービスごとに実装できるようにし、Cloud Storageへの保存を共通化することで全体の実装を簡略化しました。また、実行の制御も同じ方法で行えるようにし、定期的な実行やリトライ処理を統一的に行えるようにしました。

# 定期ログ収集・保存フレームワーク Hatchery

## 実装概要

今回の例では、ログ収集・保存ための `hatchery` というフレームワークを作成しました。 `hatchery` はGoのSDKとして実装しており、セキュリティ監視基盤を構築する際にログ収集・保存の処理を簡単に実装できるように設計されています。
...
`hatchery` では "Stream" と読んでいるデータ収集と保存のパイプラインを管理に用います。Streamはsourceとdestinationで構成されます。sourceはログデータの提供元(例: Slack、1Password、Falcon Data Replicator)で、destinationはデータ保存先(例: Google Cloud Storage、Amazon S3)になります。複数のStreamを定義し、並行して実行することができます。
...
Go言語でもロジックを外部から取り入れるために[プラグインの機構](https://pkg.go.dev/plugin)が用意されています。しかし実際にこれを使いこなそうとするとツール側とのバージョン管理や依存関係の管理が難しくなります。また、シグネチャのズレなどにも注意が必要となります。過去の実装で試したことがありましたが、運用が難しく利用を断念しました。他にも[WebAssembryを利用したプラグインの仕組み](https://knqyf263.hatenablog.com/entry/2022/08/30/052303)などもあるのですがこれはtinygoを前提としており、ログの処理において機能が十分ではない可能性を考慮して採用を見送りました。
...
現在はログをCloud Storageのみに保存していますが、この抽象化によってAmazon S3など他のオブジェクトスレーじだけでなく、Kinesis Data StreamやPub/Subに直接データを送出するということも将来的には考えられます。

## 設定値の管理

`hatchery` では設定値をハードコードもできるし、環境変数から取得すること想定しています。組織専用のバイナリを作成するため、値をそのまま埋め込んでおくこともできます。一方でクレデンシャルなどの秘匿値は環境変数から取得することで、安全に運用することができます。
...
設定値の管理においていわゆる設定ファイル（YAMLやJSON）を使うことも考えましたが、以下の様な理由で棄却しました。
...
構成についてはアーキテクチャで説明した通り、Cloud SchedularとWorkflowsを利用して定期的にCloud Run上で起動させます。図としては以下のとおりです。
