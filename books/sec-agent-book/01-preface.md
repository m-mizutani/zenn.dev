...
しかし従来のプログラミングによるソフトウェアでは、どうしても決定性の高いアプローチ[^anomaly]になり各種設定やデータの操作についてこまかく指示をコードとして表現する必要があります。それ自体は悪くありませんが、セキュリティ分析では対応する事象が多岐にわたり、それらを全て網羅するように細かくルールやロジックを設定しなければならないのは大きな負担でした。またその大量のルールをメンテナンスしないといけない、というのも問題でした。これによって自動化で得られる効果が投資に対して少なくなってしまい、セキュリティ監視と分析の効率化が滞っていました。筆者はソフトウェア開発のベストプラクティスを持ち込み、コード化の促進や自動テスト導入などでこの問題を解決しようとしましたが、結局あまりうまくはいきませんでした。
